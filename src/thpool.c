...
int thpool_add_work(thpool_* thpool_p, void (*function_p)(void*,thread*), void* arg_p){
	job* newjob;
...
static int thread_init (thpool_* thpool_p, struct thread** thread_p, int id){
	...
	*thread_p = (struct thread*)malloc(sizeof(struct thread));
	(*thread_p)->resolv_state = malloc(sizeof(struct __res_state));
	memset((*thread_p)->resolv_state, 0, sizeof(struct __res_state));
	...
	if(res_ninit((*thread_p)->resolv_state) == -1) {
		fprintf(stderr, "thread_init(): Could not initialize resolver state\n");
		return -1;
	}
...
static void* thread_do(struct thread* thread_p){
	...
	while(threads_keepalive){
		...
		if (threads_keepalive){
			...
			void (*func_buff)(void* arg, thread* thread_p);
			void* arg_buff;
			job* job_p;
			pthread_mutex_lock(&thpool_p->jobqueue_p->rwmutex);
...
static void thread_destroy (thread* thread_p){
	res_nclose(thread_p->resolv_state);
	free(thread_p->resolv_state);
    thread_p->resolv_state = NULL;
...
static void jobqueue_destroy(thpool_* thpool_p){
    ...
    thpool_p->jobqueue_p->has_jobs = 0;
}
